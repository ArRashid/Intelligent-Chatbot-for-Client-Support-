...
#while decision_maker_agent decided to use this . this is the llm with multiple promt template . it will use the relavent pormt for the task 
    ...
    def createticket(self,subject:str=None,description:str=None):
        '''When there is no ticket created already and the user says "create ticket," use this tool to execute a function that requires two inputs: subject (issue) and description (issue details)."'''
...
class TeamChatBot(BaseBot):
    ...
    def Gooogle_Search(self,quearys):
        pass
...
bot = TeamChatBot(memory=memory)

# %%
bot.load_tool_multi_pormt_assistance()
bot.load_predifine_tools(tool_names=['google-search'])
chain = bot.agent_chain_with_memory()


# %%
print(chain.run("who is pm of india now ?"))
